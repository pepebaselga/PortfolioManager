...
module Date = struct
  ...
  let check_date (d : date) : bool =
    let check_helper (d : date) =
      ...
      match d.month with
      | 2 ->
          if d.year mod 4 = 0 then d.day >= 1 && d.day <= 29
      ...
      | 10 -> long d.day
      | 8 -> long d.day
      | i -> if i mod 2 = 1 then long d.day else short d.day
    in
    match d.year with
    | x -> if x < 2000 || x > 2023 then false else check_helper d

  let make_date month day year = { month; day; year }

  let ( <=^ ) (day1 : date) (day2 : date) : bool =
    match day1.year <= day2.year with
    | false -> false
    | true -> (
        if day1.year < day2.year then true
        else
          match day1.month <= day2.month with
          | false -> false
          | true -> (
              if day1.month < day2.month then true
              else
                match day1.day <= day2.day with false -> false | true -> true))
  ...
  let ( =^ ) (day1 : date) (day2 : date) : bool = day1 <=^ day2 && day1 >=^ day2
  ...
  let of_string (s : string) =
    let (lst : string list) = Str.split (Str.regexp "/") s in
    let year = int_of_string (List.nth lst 2) in
  ...
  let to_string (d : date) =
    if check_date d = false then raise (failwith "invalid date")
    else
      let year = if d.year < 1000 then d.year + 2000 else d.year in
      string_of_int d.month ^ "/" ^ string_of_int d.day ^ "/"
      ^ string_of_int year

  let pp_date (d : date) : string =
    if check_date d = false then raise (failwith "invalid date")
    else Printf.sprintf "%i/%i/%i" d.month d.day d.year
end
...
module Candlestick = struct
  ...
  let make_cs o h l c = { openp = o; highp = h; lowp = l; closep = c }

  let to_string (cs : cs) =
    string_of_float cs.openp ^ " | " ^ string_of_float cs.highp ^ " | "
    ^ string_of_float cs.lowp ^ " | " ^ string_of_float cs.closep

  let eod_change (cs : cs) : float = cs.closep -. cs.openp
...
module Stock = struct
  ...
  let yf_to_stock (ticker : string) (csv : string list list) : stock =
    let candle = match csv with h :: t -> yf_helper t | [] -> yf_helper [] in
    (ticker, candle)
  ...
  let rec find_date_helper
      (candle : (Date.date * Candlestick.cs) list)
      (day1 : Date.date) =
    match candle with
    | [] ->
    ...
    | (d, cs) :: t ->
        if Date.compare day1 d = EQ then cs else find_date_helper t day1
