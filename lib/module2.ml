...
module Asset = struct
  ...
  (*Test*)
  let update_asset_quantity
      (portfolio : portfolio)
      (asset_name : string)
      (new_quantity : float) : portfolio =
    List.map
      (fun a ->
        if a.name = asset_name then { a with quantity = new_quantity } else a)
      portfolio

  let add_asset (portfolio : portfolio) (asset : asset) : portfolio =
    asset :: portfolio

  (* Remove an asset from the portfolio by its name (could potentially make
     longer by rewriting filter for a portfolio) *)
  let remove_asset
      (portfolio : portfolio)
      (asset_name : string)
      (day_bought : date) : portfolio =
    List.filter
      (fun a -> a.name <> asset_name || a.date_purchased <> day_bought)
      portfolio
  ...
  let make_asset
      ...
      (date_purchased : date)
      (sector : string) : asset =
    { name; quantity; purchase_price; date_purchased; sector }

  let asset_to_string asset =
    Printf.sprintf
      "[Name: %s, Quantity: %.2f, Purchase Price: %.2f, Date Purchased: %s, \
       Sector: %s]"
      asset.name asset.quantity asset.purchase_price
  ...
  let find_asset (asset_name : string) =
    List.find
      (fun x ->
        let stock_name, _ = x in
        if stock_name = asset_name then true else false)
      stock_data

  let best_dollar_asset (portfolio : portfolio) (day : date) =
    let first, first_bought =
      ( find_asset ((List.nth portfolio 0).name ^ ".csv"),
        (List.nth portfolio 0).date_purchased )
    in
    let max =
      ref
        ( (List.nth portfolio 0).name,
          (Stock.get_dollar_diff first) first_bought day )
    in
    List.iter
      (fun x ->
  ...
  let best_percent_asset (portfolio : portfolio) (day : date) =
    ...
    let max =
      ref
        ( (List.nth portfolio 0).name,
          (Stock.get_percent_diff first) first_bought day )
    in
    List.iter
      (fun x ->
        let stock = find_asset (x.name ^ ".csv") in
  ...
  let total_portfolio_value (portfolio : portfolio) (day : date) =
    ...
    List.iter
      (fun x ->
        let _, stock_v = Stock.find_date (find_asset (x.name ^ ".csv")) day in
  ...
  let total_dollarc (portfolio : portfolio) (day : date) =
    ...
    List.iter
      (fun x ->
        let stock = find_asset (x.name ^ ".csv") in
        let dollar_dif =
          x.quantity *. Stock.get_dollar_diff stock x.date_purchased day
        in
        min := !min +. dollar_dif)
      portfolio;
    !min
  ...
  let find_multisector_pc
    ...
    List.iter
      (fun x ->
        if List.mem x.sector sectors = true then
          let stock = find_asset (x.name ^ ".csv") in
          let percent = Stock.get_percent_diff stock x.date_purchased day in
          count := percent +. !count)
      portfolio;
  ...
  let particular_stock_quantity
      (portfolio : portfolio)
      (asset_name : string)
      (day : date) =
    let count = ref 0.0 in
    List.iter
      (fun x -> if x.name = asset_name then count := !count +. x.quantity)
      portfolio;
    !count
